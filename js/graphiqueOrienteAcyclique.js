...
* Inclut la gestion du thème dynamique (CSS variables) et l'édition sur double-clic.
*/
...
// === MODIFIÉ : Acceptation de la configuration existante et de la cible ===
function openDAGStudio(existingConfig = null, targetImg = null) {
    const selection = window.getSelection();
    // Sauvegarde préventive du curseur avec cloneRange
    ...
    if (existingConfig) {
        currentDAGConfig = existingConfig;
        
        // Recalcul du compteur d'ID pour éviter les collisions (ex: node_4 -> compteur = 4)
        let maxId = 1;
        currentDAGConfig.nodes.forEach(n => {
            const parts = n.id.split('_');
            if (parts.length === 2 && !isNaN(parts[1])) {
                maxId = Math.max(maxId, parseInt(parts[1]));
            }
        });
        dagNodeCounter = maxId;
    ...
    const modalHTML = `
        <div id="dag-modal" class="fr-modal fr-modal--opened" style="z-index: 100000;">
                        ...
                        <div class="fr-modal__body">
                                    ...
                                    <div class="fr-col-3">
                                        <div class="fr-p-3v" style="background: var(--grey-975); border-radius: 4px;">
                                            ...
                                            <div class="fr-select-group fr-mb-2w">
                                                ...
                                                <select id="dag-theme-select" class="fr-select fr-text--sm" onchange="changeDAGTheme(this.value)">
                                                    ${themeOptions}
                                                </select>
                                            </div>
                                            <hr class="fr-pb-1w">
                            ...
                            <div class="fr-modal__footer">
                                <div class="fr-btns-group fr-btns-group--right fr-btns-group--inline-reverse">
                                    <button id="btn-dag-insert" class="fr-btn" onclick="finalizeAndInsertDAG()">Générer et Insérer</button>
    ...
    // Initialisation de l'interface avec les données existantes
    const themeSelect = document.getElementById('dag-theme-select');
    if (themeSelect) themeSelect.value = currentDAGConfig.theme || "dynamique";

    const btnInsert = document.getElementById('btn-dag-insert');
    if (existingConfig && btnInsert) {
        btnInsert.textContent = "Mettre à jour le logigramme";
        btnInsert.targetContainer = targetImg; // Stockage de l'image cible sur le bouton
    }
...
// === MODIFIÉ : Routage vers création ou mise à jour ===
async function finalizeAndInsertDAG() {
    isLinkModeActive = false;
    selectedSourceNodeId = null;
    
    // Récupération de la cible éventuelle (Mode Mise à Jour)
    const btnInsert = document.getElementById('btn-dag-insert');
    const targetImg = btnInsert ? btnInsert.targetContainer : null;
    ...
    try {
        ...
        if (targetImg) {
            ...
            targetImg.setAttribute('data-dag-config', configJSON);
        } else {
...
// --- MOTEUR DE RAFRAÎCHISSEMENT THEME DYNAMIQUE ---
...
document.addEventListener('dblclick', (e) => {
    // On cible spécifiquement l'image contenant la configuration
    const imgTarget = e.target.closest('img.plume-dag-image');
    
    if (imgTarget) {
        e.preventDefault(); 
        try {
            const rawConfig = imgTarget.getAttribute('data-dag-config');
            let config;
            try {
                // Décodage URI standard sécurisé
                config = JSON.parse(decodeURIComponent(rawConfig));
            } catch (err) {
                // Fallback (Rétrocompatibilité anciens schémas)
                config = JSON.parse(rawConfig.replace(/&#39;/g, "'"));
            }
            
            // Relance l'éditeur avec la configuration et l'image cible à remplacer
            openDAGStudio(config, imgTarget);
        } catch (err) {
            console.error("Erreur lors de la restauration du logigramme DAG :", err);
        }
    }
});
